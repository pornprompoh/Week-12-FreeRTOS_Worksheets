...
|-------|-----------|-----------|--------------|----------|---------------------|
| 1 (3R, 5P) | 3 | 5 |80% |150 ms | 90%|
| 2 (5R, 5P) | 5 | 5 |100% |50 ms |95% |
| 3 (3R, 8P) | 3 | 8 |60% |300 ms | 100%|

### คำถามสำหรับการทดลอง
1. เมื่อ Producers มากกว่า Resources จะเกิดอะไรขึ้น?
```
เกิด คอขวด (bottleneck) เพราะ Producers ต้องรอให้ Resource ว่าง
```
2. Load Generator มีผลต่อ Success Rate อย่างไร?
```
หาก Load สูง (ส่งคำขอมาก / Producer เยอะ) → Success Rate ลดลงเพราะบางคำขอถูกบล็อกหรือต้องรอ
```
3. Counting Semaphore จัดการ Resource Pool อย่างไร?
```
Counting Semaphore นับจำนวน Resource ที่ว่าง
เมื่อ Task ขอ Resource → xSemaphoreTake() ลดค่า count ลง
เมื่อ Task คืน Resource → xSemaphoreGive() เพิ่มค่า count ขึ้น
```
## 📋 สรุปผลการทดลอง
