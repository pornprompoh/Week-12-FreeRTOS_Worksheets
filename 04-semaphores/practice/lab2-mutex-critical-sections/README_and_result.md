...
|-------|------------|--------|-----------|-------------|---------|
| 1 (With Mutex) |50 | 0| 0| 100%|การเข้าถึง shared resource ปลอดภัย ไม่มีปัญหา data corruption |
| 2 (No Mutex) |30 |0 | 20| 60%| เกิด data corruption เพราะหลาย Task เข้าถึง resource พร้อมกัน|
| 3 (Changed Priority) | 50| 0|0 |100% | Priority Inheritance ช่วยป้องกัน priority inversion ทำให้ Task สำเร็จทั้งหมด|
...
```
เมื่อ Task ความสำคัญสูง ต้องการ resource ที่ Task ต่ำกว่ากำลังถือ Mutex → FreeRTOS จะ ยก priority ของ Task ต่ำขึ้นเท่ากับ Task สูง ชั่วคราว
หลังจาก Task ต่ำปล่อย Mutex → priority คืนสู่ค่าเดิม
```
3. Task priority มีผลต่อการเข้าถึง shared resource อย่างไร?
```
Task ที่มี priority สูง จะได้สิทธิ์เข้าถึง resource ก่อน หาก resource ว่าง
```
## 📋 สรุปผลการทดลอง
