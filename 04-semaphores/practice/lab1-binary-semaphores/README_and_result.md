...
|-------|-------------|-----------------|--------------|----------------|------------|
| 1 (Normal) | 50| 50|10 | 5| 100%|
| 2 (Multiple Give) |100 |75 |12 | 8| 75%|
| 3 (Short Timeout) |50 |40 | 10| 5|80% |

### คำถามสำหรับการทดลоง
1. เมื่อ give semaphore หลายครั้งติดต่อกัน จะเกิดอะไรขึ้น?
```
ถ้าเป็น Binary Semaphore:ค่าจะถูกตั้งเป็น available ครั้งเดียวเท่านั้นการ give เพิ่มอีกหลายครั้ง จะไม่สะสม → เมื่อ Task รับ semaphore แล้ว เหลือค่าเดียวเหมือนเดิม
ถ้าเป็น Counting Semaphore:ค่า count จะเพิ่มขึ้นตามจำนวน giveTask หลายตัวสามารถรับค่าได้หลายครั้ง
```
2. ISR สามารถใช้ `xSemaphoreGive` หรือต้องใช้ `xSemaphoreGiveFromISR`?
```
ใน ISR ต้องใช้ xSemaphoreGiveFromISR() เท่านั้นเพราะ ISR ทำงานใน interrupt context
```
3. Binary Semaphore แตกต่างจาก Queue อย่างไร?
```
Binary Semaphore ไม่มีข้อมูล, แค่ให้/รับ
Queue เก็บข้อมูลจริง (structure, int, etc.)
```
## 📋 สรุปผลการทดลอง
