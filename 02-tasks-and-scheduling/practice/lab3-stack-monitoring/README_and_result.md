...
```
Task ที่มีการใช้ตัวแปรภายใน (local variables) เยอะ หรือเรียกฟังก์ชันซ้อนกันหลายชั้น จะใช้ stack มากที่สุด
```
2. การใช้ heap แทน stack มีข้อดีอย่างไร?
```
ใช้พื้นที่เฉพาะตอนจำเป็น จัดการหน่วยความจำได้ยืดหยุ่น
```
3. Stack overflow เกิดขึ้นเมื่อไหร่และทำอย่างไรป้องกัน?
```
เกิดเมื่อ: Task ใช้ stack เกินขนาดที่กำหนดตอนสร้าง
วิธีป้องกัน เปิดการตรวจจับใน menuconfig
```
4. การตั้งค่า stack size ควรพิจารณาจากอะไร?
```
จำนวนตัวแปรในฟังก์ชัน การเรียกฟังก์ชันซ้อนกัน การใช้ฟังก์ชันที่กิน stack มาก เช่น printf() ความซับซ้อนของ task
```
5. Recursion ส่งผลต่อ stack usage อย่างไร?
```
Recursion (การเรียกฟังก์ชันตัวเอง) ใช้ stack เพิ่มทุกครั้งที่เรียกซ้ำ
```
