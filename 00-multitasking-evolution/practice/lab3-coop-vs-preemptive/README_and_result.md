...
```
Preemptive Multitasking มีเวลาตอบสนองดีกว่า เพราะ: ระบบสามารถบังคับสลับไปยังงานอื่นได้ทันทีตาม priority และ time slice ไม่ต้องรอให้งานปัจจุบันสละ CPU เอง
```
2. ข้อดีของ Cooperative Multitasking คืออะไร?
```
โครงสร้าง เรียบง่าย → ไม่มี overhead จากการสลับบังคับ
```
3. ข้อเสียของ Cooperative Multitasking คืออะไร?
```
ถ้างานใดงานหนึ่ง ไม่ยอมคืน CPU → งานอื่นจะถูก block ไปทั้งหมด
```
4. ในสถานการณ์ใดที่ Cooperative จะดีกว่า Preemptive?
```
ระบบที่ เล็กและง่าย เช่น embedded เล็ก ๆ, microcontroller ที่รัน task ไม่กี่งาน
```
5. เหตุใด Preemptive จึงเหมาะสำหรับ Real-time systems?
```
เพราะสามารถ บังคับสลับไปทำงานที่สำคัญกว่าได้ทันที
```
