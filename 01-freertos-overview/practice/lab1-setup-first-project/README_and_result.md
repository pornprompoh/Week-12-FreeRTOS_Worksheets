...
- [ใช่] ตรวจสอบ ESP-IDF environment สำเร็จ
- [ใช่] สร้างโปรเจกต์ใหม่ได้
- [ใช่] เข้าใจโครงสร้างโปรเจกต์
- [ใช่] Build โปรเจกต์สำเร็จ
- [ใช่] แก้ไขโค้ดและ build ใหม่ได้
- [ใช่] เข้าใจไฟล์ CMakeLists.txt
- [ใช่] ทำแบบฝึกหัดครบ
...
```
CMakeLists.txt – บอกระบบ build ว่าจะคอมไพล์ไฟล์ใดบ้าง
main/CMakeLists.txt – ระบุไฟล์ source ภายในโฟลเดอร์ main/
main/main.c – ไฟล์โค้ดหลักของโปรแกรม

```
2. ความแตกต่างระหว่าง `hello_esp32.bin` และ `hello_esp32.elf` คืออะไร?
```
.elf = สำหรับดีบัก
.bin = สำหรับอัปโหลดลงบอร์ด
```
3. คำสั่ง `idf.py set-target` ทำอะไร?
```
ใช้ กำหนดชนิดของชิป ESP ที่จะคอมไพล์ให้ตรงกับฮาร์ดแวร์
```
4. โฟลเดอร์ `build/` มีไฟล์อะไรบ้าง?
```
hello_esp32.bin — ไฟล์ที่พร้อมแฟลช
hello_esp32.elf — ไฟล์สำหรับดีบัก
project_description.json — รายละเอียดโปรเจกต์
CMakeFiles/, esp-idf/, bootloader/ — ไฟล์ที่ระบบ build สร้างขึ้นระหว่างคอมไพล์
flasher_args.json — พารามิเตอร์สำหรับแฟลชบอร์ด
```
5. การใช้ `vTaskDelay()` แทน `delay()` มีความสำคัญอย่างไร?
```
ESP-IDF ใช้ระบบ FreeRTOS multitasking
พักเฉพาะ task ปัจจุบัน แล้วให้ CPU ไปทำงานกับ task อื่น
```
