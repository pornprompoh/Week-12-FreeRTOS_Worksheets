...
|-------|-------------|---------------|--------------|---------|
| 1 | 2s | 1.5s | Normal | ส่งและรับได้ต่อเนื่อง Queue ไม่เต็มไม่ว่าง|
| 2 | 0.5s | 1.5s | Full | Sender ส่งเร็วกว่า Receiver → Queue เต็มบ่อย|
| 3 | 2s | 0.1s | Empty | Receiver อ่านเร็วกว่า Sender → Queue มักว่าง|

### คำถามสำหรับการทดลอง
1. เมื่อ Queue เต็ม การเรียก `xQueueSend` จะเกิดอะไรขึ้น?
```
ถ้า ไม่มี timeout → ฟังก์ชัน xQueueSend() จะ คืนค่า fail (pdFALSE) ทันที เพราะไม่มีที่ว่าง
ถ้า มี timeout → Task จะ เข้า Blocked state เพื่อรอจนกว่า Queue จะว่าง หรือเวลาที่กำหนดหมดลง
```
2. เมื่อ Queue ว่าง การเรียก `xQueueReceive` จะเกิดอะไรขึ้น?
```
ถ้า ไม่มี timeout → xQueueReceive() จะ คืนค่า fail (pdFALSE) เพราะไม่มีข้อมูลให้อ่าน
ถ้า มี timeout → Task จะ เข้า Blocked state เพื่อรอจนกว่าจะมีข้อมูลใน Queue หรือเวลาหมด
```
3. ทำไม LED จึงกะพริบตามการส่งและรับข้อความ?
```
เพราะ LED ถูกสั่งให้เปลี่ยนสถานะ (ON/OFF) ภายใน task ของ Sender และ Receiver ทุกครั้งที่มีการส่งหรือรับข้อมูลจาก Queue
```
