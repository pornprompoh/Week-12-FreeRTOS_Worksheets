...
|-------|--------|------|---------|-------|-------|---------|
| 1 (ปกติ) | ✅ ส่งข้อมูลทุก 2s |✅ ส่งคำสั่งทุก 5s | ✅ รับค่าทุก 3s| ✅ ทำงานทุก 1s| 100%| ระบบทำงานสมดุล Processor รับข้อมูลสม่ำเสมอ|
| 2 (ไม่มี Sensor) |❌ ไม่ส่งข้อมูล | ✅ ส่งคำสั่งทุก 5s|✅ รับค่าทุก 3s | ✅ ทำงานทุก 1s| 70%|Processor ว่างบางช่วง แต่ยังคงทำงานจาก Queue อื่น |
| 3 (Network เร็ว) | ✅ ส่งข้อมูลทุก 2s|✅ ส่งคำสั่งทุก 5s |⚡ ส่งข้อมูลทุก 0.5s |✅ ทำงานทุก 1s | 120%|Processor ถูกโหลดสูงขึ้นเพราะ Network ส่งบ่อยมาก |

### คำถามสำหรับการทดลอง
1. Processor Task รู้ได้อย่างไรว่าข้อมูลมาจาก Queue ไหน?
```
ใช้ Queue Sets ซึ่งรวมหลาย Queue เข้าด้วยกัน เมื่อมีข้อมูลเข้ามาใน Set ใด Set หนึ่ง xQueueSelectFromSet() จะคืนค่า handle ของ Queue ที่มีข้อมูล
```
2. เมื่อหลาย Queue มีข้อมูลพร้อมกัน เลือกประมวลผลอันไหนก่อน?
```
FreeRTOS จะเลือก Queue ที่พร้อมก่อนตามลำดับภายใน Queue Set ถ้ามีพร้อมพร้อมกันหลายตัว → ระบบจะเลือกตัวที่อยู่ใน Set ก่อนสุดที่มีข้อมูล
```
3. Queue Sets ช่วยประหยัด CPU อย่างไร?
```
ปกติ Task ต้อง poll (ตรวจสอบ) ทุก Queue ว่ามีข้อมูลหรือไม่ แต่เมื่อใช้ Queue Set → Processor Task รอแค่จุดเดียว (xQueueSelectFromSet())
```
## 📋 สรุปผลการทดลอง
