...
```
เนื่องจาก มี Producer มากกว่า Consumer (4:2) → ข้อมูลถูกผลิตเร็วเกินไป จน Queue เต็มบ่อยครั้ง
```
2. ในทดลองที่ 3 ระบบทำงานเป็นอย่างไร?
```
มี Consumer เพียงตัวเดียว ทำให้การบริโภคข้อมูลช้ากว่าการผลิต → Queue ค่อย ๆ เต็ม
```
3. Load Balancer แจ้งเตือนเมื่อไหร่?
```
Load Balancer จะ แจ้งเตือนเมื่อ Queue มีการใช้งานเกิน Threshold ที่กำหนด เช่น ใช้พื้นที่เกิน 80% ของขนาด Queue
```
